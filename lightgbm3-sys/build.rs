...
fn main() {
    ...
    let cfg = cfg
        .profile("Release")
        .uses_cxx11()
        .cxxflag("-std=c++17") // updated to c++17 from 11
    ...
    let bindings = bindgen::Builder::default()
        .header("wrapper.h")
        .clang_args(&["-x", "c++", "-std=c++17"]) // updated to c++17 from 11
        .clang_arg(format!("-I{}", lgbm_root.join("include").display()))
        // had to disable bunch of things to make it running at 4.3
        .opaque_type("std::.*")
        ...
        // END had to disable bunch of things to make it running at 4.3
        .generate()
